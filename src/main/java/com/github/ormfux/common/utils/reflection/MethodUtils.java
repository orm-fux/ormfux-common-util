...
import static com.github.ormfux.common.utils.NullableUtils.check;
import static com.github.ormfux.common.utils.NullableUtils.equalTo;
...
public final class MethodUtils {
    ...
    public static Method findPropertySetter(final Class<?> entityClass, final String property, final Class<?> propertyType) throws NoSuchMethodException {
        final String setterName = "set" + capitalize(property);
        ...
        if (nonNull(propertyType)) {
            ...
            if (check(setterMethod, Method::getModifiers, Modifier::isPublic)) {
                return setterMethod;
            }
        } else {
            final Method[] methods = entityClass.getMethods();
    ...
    public static Method findPropertyGetter(final Class<?> entityClass, final String property, final Class<?> propertyType) throws NoSuchMethodException {
        ...
        final String getterName = "get" + capitalize(property);
        final String booleanGetterName = "is" + capitalize(property);
        ...
        if (check(getterMethod, Method::getModifiers, Modifier::isPublic)) {
    ...
    public static Method findPropertyGetter(Class<?> entityClass, String property) throws NoSuchMethodException {
        ...
        if (check(getterMethod, Method::getReturnType, equalTo(Boolean.TYPE))) {
            try {
    ...
    private static String capitalize(final String value) {
        if (check(value, not(String::isEmpty))) {
